...
이미 디자인팀은 **Drive 공유 드라이브**에 패턴을 잘 정리해두고 있다 (`G:\공유 드라이브\디자인\2026 커스텀용 패턴\00. SVG\` 하위). 이 자산을 grader 앱이 직접 읽어 쓰면:
...
| 코드 변경량 | 거의 0 (심볼릭 링크) | 중간 (+640줄, 폴더 스캔 로직 신규) |
...
### 한 줄 요약
폴더는 "종목 → 세부 카테고리" 2단계, 파일명은 `{패턴명}_{사이즈}.svg` 또는 `{패턴명}_{조각명}_{사이즈}.svg` 두 가지 패턴만 인정한다.

### 카테고리 깊이 규칙

**현재 확인된 구조 (depth 2):**
```
G:\공유 드라이브\디자인\2026 커스텀용 패턴\00. SVG\   ← 루트 (사용자가 Settings에서 지정)
├── 농구유니폼\                                       ← depth 0 = 카테고리 "농구유니폼"
│   ├── 1. 단면 유니폼 상의 패턴\                      ← depth 1 = 카테고리 "단면 유니폼 상의 패턴"
│   │   ├── 농구유니폼_V넥_스탠다드_암홀X_XS.svg    ← 패턴 파일
│   │   ├── 농구유니폼_V넥_스탠다드_암홀X_S.svg
│   │   └── ... (13 사이즈)
│   ├── 2. 양면 유니폼 상의 패턴\
│   └── 3. 하의 패턴\
├── 축구유니폼\
...
**규칙:**
- 최대 **2레벨**까지 카테고리로 인정 (depth 0: 종목, depth 1: 세부)
- 3레벨 이상 폴더가 발견되면 **경고 로그 + 평탄화** (현 시점 기본값. 사용자 확인 [J-3] 필요)
- 폴더명 앞의 `1. ` `2. ` 같은 정렬용 번호는 **그대로 카테고리명에 포함**한다 (단순화). 추후 정렬 키와 표시명을 분리하려면 별도 검토.

### 파일명 파싱 규칙

**패턴 1: 단일 조각** (조각 1개 = 파일 1개)
```
{패턴명}_{사이즈}.svg
예: 농구유니폼_V넥_스탠다드_암홀X_XS.svg
    └─────────── 패턴명 ───────────┘ └사이즈┘
```

**패턴 2: 다중 조각** (조각 N개 = 파일 N개, 사용자 확인 [J-2] 가정)
```
{패턴명}_{조각명}_{사이즈}.svg
예: 농구V넥_앞판_XS.svg
    농구V넥_뒷판_XS.svg
...
**파싱 알고리즘 (의사코드)**:
```typescript
function parseFileName(fileName: string): ParseResult | null {
  // 1. 확장자 제거
  const base = fileName.replace(/\.svg$/i, "");

  // 2. _ 로 분리
  const tokens = base.split("_");
  if (tokens.length < 2) return null; // 최소 패턴명_사이즈

  // 3. 마지막 토큰이 사이즈인지 확인
  const lastToken = tokens[tokens.length - 1];
  if (!SIZE_WHITELIST.includes(lastToken)) return null;

  // 4. 끝에서 두 번째 토큰이 "조각명 후보"인지 확인
  //    (사용자 정의 조각명 사전 또는 휴리스틱)
  const secondLast = tokens[tokens.length - 2];
  const isPiece = PIECE_NAME_HINTS.includes(secondLast); // 예: ["앞판","뒷판","소매","칼라","..."]

  if (isPiece) {
    return {
      patternName: tokens.slice(0, -2).join("_"),
      pieceName: secondLast,
      size: lastToken,
    };
  } else {
    return {
      patternName: tokens.slice(0, -1).join("_"),
      pieceName: "default", // 단일 조각
      size: lastToken,
    };
  }
}
...
### 그룹핑 규칙
같은 폴더 안에서 `patternName`이 같은 파일들 = **하나의 PatternPreset**.
- `pieceName` 종류 수 = 조각 수
- `size` 종류 수 = 지원 사이즈 수
...
├── 농구유니폼_V넥_스탠다드_암홀X_S.svg   │
├── ...                                  │ → PatternPreset
└── 농구유니폼_V넥_스탠다드_암홀X_5XL.svg ┘   "농구유니폼_V넥_스탠다드_암홀X"
                                              조각 1개 × 사이즈 13개 = 13파일
```
...
{
  "stableId": "preset-2026-abc123",     // UUID — 폴더명 바뀌어도 식별 유지
  "createdAt": "2026-04-15T10:00:00Z",
  "displayName": "농구 V넥 스탠다드",      // 폴더명 대신 표시할 별칭 (선택)
  "pieces": [                            // 조각 사전 (다중 조각일 때 자동 생성)
    {"name": "default", "label": "본판"}
  ],
...
export interface AppSettings {
  drivePatternRoot?: string;     // Drive 루트 절대 경로 (예: "G:\\공유 드라이브\\디자인\\...\\00. SVG")
...
### 한 줄 요약
신규 4개 파일 + 수정 7개 파일, 총 약 640줄.

| 파일 | 종류 | 역할 (한 줄) | 예상 줄수 |
|------|------|------|---------|
| `src/services/driveSync.ts` | 신규 | 폴더 트리 스캔 + 파일명 파싱 + meta.json 읽기/쓰기 | 200 |
...
| `src/stores/categoryStore.ts` | 수정 | 폴더 트리 → 카테고리 자동 생성 | +40 |
...
**총 신규 약 410줄 + 수정 약 230줄 = 약 640줄** (주석 포함)
...
### 한 줄 요약
타입 확장 → 스캔 서비스 → 캐시/설정 → UI → 통합 머지의 순서. 각 단계는 5~30분 단위로 쪼개져 있어 중간에 멈춰도 안전하다.

| # | 작업 | 산출물 | 담당 | 선행 | 예상 시간 |
|---|------|-------|------|------|---------|
| 1 | `src/types/pattern.ts` 확장 | 타입 정의 | developer | — | 15분 |
| 2 | Tauri `fsScope` 설정 (`G:\` 읽기 허용) | tauri.conf.json + capabilities | developer | — | 15분 |
| 3 | `src/services/driveSync.ts` 초안 — `scanDriveRoot(path)` | 스캔 함수 (조각 1개 케이스만) | developer | 1, 2 | 60분 |
| 4 | 단위 검증: 모의 폴더 만들어 스캔 결과 콘솔 출력 | PASS/FAIL | tester | 3 | 20분 |
...
| 8 | `driveSync.ts` 확장 — 다중 조각 파싱 (`{패턴}_{조각}_{사이즈}`) | 파싱 분기 | developer | 3 | 45분 |
| 9 | `meta.json` 읽기/자동 생성 로직 | UUID 발급 | developer | 3 | 30분 |
| 10 | `src/stores/categoryStore.ts` 자동 생성 | 폴더 트리 → 카테고리 | developer | 3 | 30분 |
| 11 | `src/stores/presetStore.ts` 머지 로직 | stableId 기반 중복 방지 | developer | 9, 10 | 45분 |
| 12 | `src/pages/PatternManage.tsx` "가져오기" 버튼 + 미리보기 | UI 통합 | developer | 7, 11 | 60분 |
| 13 | 통합 테스트 (실제 Drive 폴더 지정 → 스캔 → 임포트) | E2E PASS | tester | 12 | 30분 |
| 14 | reviewer 코드 리뷰 (병렬 가능) | 리뷰 결과 | reviewer | 12 | 30분 |

**총 약 8~9시간 (3~4일 분산 작업 가정)**.

### 병렬 실행 가능 구간
- 5, 6 동시 진행 가능 (서로 독립)
- 13, 14 동시 진행 가능 (tester + reviewer)
...
- Drive for Desktop 설치 후 Windows 탐색기에서 다음 경로가 보이는지 확인:
  ```
  G:\공유 드라이브\디자인\2026 커스텀용 패턴\00. SVG\
  ```
...
3. `G:\공유 드라이브\디자인\2026 커스텀용 패턴\00. SVG` 선택
4. 저장 → "Drive 동기화 활성화" 토글 ON
5. **PatternManage** 페이지 → "Drive에서 가져오기" 버튼 → 미리보기 확인 → 임포트

### 4. (Phase 2 이후) 새 패턴 추가 워크플로우
- **방법 A — Drive에 직접 SVG 업로드**: 디자이너가 `G:\...\농구유니폼\1. 단면.../농구신상_XS.svg` 올림 → grader 자동 감지 → 카테고리 트리에 자동 표시
...
| T3 | 다중 조각 패턴 (사용자 확인 [J-2] 가정 후) | `농구V넥_앞판_{13}` + `농구V넥_뒷판_{13}` | 1개 PatternPreset, pieces 배열 길이 2 |
...
### 한 줄 요약
폴더 트리 확정 후 7개 항목 답변 필요. **굵은 글씨 항목은 코드 작성 전 반드시 확정** 필요.

```
☐ [J-1] ✅ 단일 조각 패턴 파일명 규칙: {패턴명}_{사이즈}.svg
        → 이미 확인됨 (코드 진행 가능)

☐ [J-2] ⚠️ 다중 조각 패턴 파일명 규칙
        가정: 후보 A "{패턴명}_{조각명}_{사이즈}.svg"
        예: 농구V넥_앞판_XS.svg, 농구V넥_뒷판_XS.svg
        → 실제 사용 규칙 확인 필요 (B/C/D 후보 중 선택 또는 새 규칙)

☐ [J-3] ⚠️ 카테고리 깊이 제한
        가정: 최대 2레벨 (종목 → 세부)
        → 3레벨 이상 들어갈 계획 있는지? (예: 종목 → 부위 → 스타일)

☐ [J-4] ⚠️ 확정된 Drive 루트 경로
        가정: G:\공유 드라이브\디자인\2026 커스텀용 패턴\00. SVG\
        → 폴더 트리 변경 후 최종 경로 알려줄 것

☐ [J-5] 앱에서 업로드할 때 자동 저장 규칙 (Phase 2)
        - 어느 카테고리 폴더에 저장? (사용자가 UI에서 선택? 기본 폴더?)
        - 파일명 자동 생성? (프리셋 이름 + 사이즈 조합)
        - 다중 조각인 경우 조각명 입력 받기?

☐ [J-6] 파일명 중복 정책 (Phase 2)
        - 이미 같은 이름 파일이 Drive에 있을 때 어떻게 처리?
          (덮어쓰기 / 거부 / 자동 번호 추가)

☐ [J-7] meta.json 자동 생성 동의
...
| 단계 | 기간 | 담당 | 비고 |
|------|------|------|------|
| Phase 0 (폴더 트리 확정 대기) | 사용자 작업 | 사용자 | [J-1~4] 확정 후 착수 가능 |
| Phase 1 (MVP 읽기 전용) | 3~4일 | developer + tester + reviewer | 약 8~9시간 실작업 |
| Phase 2 (양방향) | 2~3일 | developer + tester | Phase 1 안정화 1주 후 권장 |
...
### 사용자
1. 폴더 트리 변경 작업 완료 후 [J-2~4] 답변
2. (선택) [J-7] meta.json 자동 생성 동의 여부 결정

### planner-architect (이 에이전트)
- 사용자 답변 수신 후 본 PLAN의 [B], [C] 섹션 업데이트
- 필요 시 미리보기 모달 와이어프레임 추가

### developer
- Phase 0 대기 중 (착수 금지)
- [J-1~4] 확정 후 [E] 표 1번부터 순차 진행
- 각 단계별로 사용자 승인 받고 진행 (CLAUDE.md "코드 수정 전 반드시 확인" 원칙 준수)

---

> **이 PLAN은 Phase 0 (폴더 트리 확정 대기) 상태입니다.**
> 사용자 [J-2~4] 답변 후 PLAN 갱신 → 사용자 최종 승인 → developer 착수.
