...
> **작성일**: 2026-04-15 (초안) / 2026-04-15 (J-2/J-3/J-4 반영 미세 조정) / 2026-04-15 (실측 구조 반영 — 깊이 제한 없음)
...
> **확정 파라미터**: 루트 = `G:\공유 드라이브\디자인\00. 2026 커스텀용 패턴 SVG` / 카테고리 깊이 제한 없음 (안전 상한 20, 실측 최대 6레벨) / 조각 1개 가정 (Phase 1)

## 갱신 이력
- 2026-04-15 (초안): Phase 1 작성 (커밋 fbe0ae3)
- 2026-04-15 (미세조정 1): J-2/J-3/J-4 반영 (커밋 eb477b7)
- 2026-04-15 (미세조정 2): 실측 구조 반영 — 깊이 제한 없음, 실측 최대 6레벨 확인 (이 커밋)
...
### 한 줄 요약
폴더는 **깊이 제한 없이** 재귀 스캔되는 카테고리 트리, 파일명은 `{패턴명}_{사이즈}.svg` 하나의 규칙만 인정한다 (조각 1개 가정, Phase 1 범위).

### 카테고리 깊이 규칙 (J-3 확정: 깊이 제한 없음)

**실측된 구조 (최대 depth 6 예시):**
```
G:\공유 드라이브\디자인\00. 2026 커스텀용 패턴 SVG\           ← 루트 (사용자가 Settings에서 지정)
└── 0. 농구유니폼 확정 정리본\                                 ← depth 0 = 카테고리
    └── 1. 단면 유니폼 상의 패턴\                              ← depth 1 = 카테고리
        └── V넥\                                              ← depth 2 = 카테고리
            └── 슬림\                                         ← depth 3 = 카테고리
                └── V넥 상의 슬림(암홀X)\                     ← depth 4 = 프리셋 폴더
                    ├── 농구유니폼_V넥_스탠다드_암홀X_XS.svg  ← depth 5 = 패턴 파일
                    └── ... (13 사이즈)
```

**규칙:**
- **깊이 제한 없음** — 카테고리 트리는 몇 단계든 허용 (driveSync.ts 내부 안전 상한은 20, 무한 재귀 방어용)
- 실전 최대는 **6레벨** 정도로 관찰됨 (루트→카테고리 4~5단→프리셋 폴더→SVG)
- 각 레벨은 **선택적** — 어느 depth에서든 바로 SVG 파일이 나와도 OK
...
- **참고**: 카테고리 UI가 깊어지면 가독성이 떨어질 수 있으므로 Phase 2에서 트리 접기/펼치기 UX 개선 고려 필요
...
| 파일 | 종류 | 역할 (한 줄) | 예상 줄수 |
|------|------|------|---------|
| `src/services/driveSync.ts` | 신규 | 폴더 트리 재귀 스캔(깊이 제한 없음, 안전 상한 20) + 파일명 파싱(단일 규칙) + meta.json 읽기/쓰기 | 170 |
...
| `src/stores/categoryStore.ts` | 수정 | 폴더 트리 → 카테고리 자동 생성 (깊이 무제한 지원) | +45 |
...
| 3 | `src/services/driveSync.ts` 초안 — `scanDriveRoot(path)` (깊이 무제한 재귀, 안전 상한 20) | 스캔 함수 (단일 규칙 파서) | developer | 1, 2 | 60분 |
| 4 | 단위 검증: 모의 폴더(심층 구조) 만들어 스캔 결과 콘솔 출력 | PASS/FAIL | tester | 3 | 20분 |
...
| 9 | `src/stores/categoryStore.ts` 자동 생성 (깊이 무제한 지원) | 폴더 트리 → 카테고리 | developer | 3 | 30분 |
...
| T3 | 심층 카테고리(6레벨+) 정상 스캔 | 루트/카테고리/세부/V넥/슬림/프리셋폴더/농구V넥_XS.svg | 모든 카테고리 노드 생성 + 최하위 프리셋 폴더에 프리셋 소속 |
...
✅ [J-3] 카테고리 깊이 제한
        → 확정: **깊이 제한 없음** (사용자 요청 "제한 없이")
        → driveSync.ts 내부 안전 상한은 20레벨 (무한 재귀 방어용, 실전 발동 거의 없음)
        → 실측 최대 6레벨 확인 (루트/카테고리/세부/V넥/슬림/프리셋폴더/SVG)
        → 카테고리 UI가 깊어지면 가독성 저하 → Phase 2에서 트리 UX 개선 고려
