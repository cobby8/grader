...
- **내용**: AI 파일 처리 시 변환 도구 선택 방식. **채택 (A)**: `open(f, 'rb').read(10)` 첫 10바이트가 `%PDF-` 시작이면 PyMuPDF 경로, `%!PS-Adobe` 시작이면 PostScript 분류(Phase 2 skip), 그 외는 unknown 에러. **거부 (B)**: 파일 확장자만 사용 + try/except로 PyMuPDF 시도 — 11% PostScript가 try 안에서 큰 오류 스택 트레이스 찍고 실패, 로그 오염 + UX 혼란. **거부 (C)**: 외부 `file` 명령 subprocess — Windows 기본 환경 미보장 + 오버헤드. **근거**: (1) 2026-04-20 외부 63개 실증에서 100% 분류 정확도 확인(PyMuPDF 56 + Illustrator COM 7), (2) Adobe PostScript/PDF 헤더 규약은 수십 년 안정, (3) O(1) 비용에 확정적 판정, (4) lessons.md `[2026-04-21]` 항목이 이 규칙을 명시적으로 교훈으로 기록함.
- **참조횟수**: 0

### [2026-04-25] AI→SVG 실행 방식: 반자동 (Drive 스캔 후 배너 → 사용자 클릭)
- **분류**: decision
- **발견자**: 사용자 확정
- **내용**: AI 변환 트리거 방식. **채택**: 반자동 — Drive 스캔 완료 후 "미변환 AI N개 발견" 배너만 띄우고, 사용자가 [변환] 버튼을 직접 눌러야 실행. **거부 (자동)**: Drive 스캔과 동시에 조용히 변환 — Phase 3에서 옵트인 형태로 재검토. **거부 (완전 수동/파일 탐색기 업로드)**: 배너 없이 메뉴에서 명시적 트리거 — 발견성 저하. **근거**: (1) 투명성 우선 — 사용자가 G드라이브 원본 파일이 자동 수정되는 것을 통제할 수 있어야 함, (2) "차근차근" 바이브 코더 원칙과 정합, (3) 배너 방식은 "발견성 + 통제감"을 동시 만족, (4) Phase 3에서 옵트인 자동화는 기반 기능이 안정된 후 덧붙이는 것이 안전.
- **참조횟수**: 0

### [2026-04-25] AI→SVG 변환 저장 위치: G드라이브 동일 폴더 (별도 폴더/다운로드 거부)
- **분류**: decision
- **발견자**: 사용자 확정
- **내용**: 변환된 SVG 저장 경로. **채택**: AI 파일과 같은 G드라이브 폴더(`XL.ai` 옆에 `XL.svg` 생성). **거부 (A)**: 별도 `converted/` 하위 폴더 — 디자이너가 SVG가 어디 있는지 혼란. **거부 (B)**: 사용자 다운로드 폴더 — Drive 스캔이 인식 못함, 수동 업로드 부담 복귀. **거부 (C)**: 앱 AppData에 복사 후 링크 — 2중 저장 + 원본 위치 깨짐. **근거**: (1) Drive 스캔이 폴더 단위로 순회하므로 동일 폴더면 자연스럽게 인식, (2) 기존 SVG 표준화와 동일 정책(G드라이브 직접 쓰기 허용), (3) 사용자의 수동 흐름 "AI 올림 → 앱 자동 변환 → 즉시 등록"이 중단 없음, (4) atomic write(.tmp→rename)로 파일 손상 방지.
- **참조횟수**: 0

### [2026-04-25] AI→SVG 충돌 처리: 기본 건너뜀 + 옵션 덮어쓰기(.bak 백업)
- **분류**: decision
- **발견자**: 사용자 확정 (SVG 표준화 UX 패턴 재사용)
- **내용**: 같은 이름 SVG가 이미 존재할 때 처리. **채택**: 기본값은 skip(덮어쓰기 안 함), 모달 체크박스로 "덮어쓰기 (.bak 자동 생성)" 옵션 제공. **거부 (A)**: 항상 덮어쓰기 — 사용자가 수동 수정한 SVG 손실 위험. **거부 (B)**: 매번 파일별 확인 팝업 — 배치 처리 시 수십 번 클릭. **거부 (C)**: 이름 충돌 시 자동 rename(`XL_2.svg`) — Drive 스캔이 인식 못함 + 혼란. **근거**: (1) SVG 표준화의 `.bak` 백업 패턴이 이미 검증됨, (2) 실제로 Phase 1 배너 경로(짝 없는 AI만 처리)에서는 충돌이 발생할 수 없음 — "짝 있는 AI"는 처음부터 `unconvertedAiFiles`에 포함되지 않음. 따라서 Phase 1에서 덮어쓰기는 사실상 엣지 케이스. (3) UI 옵션으로 노출은 일관성 유지(Phase 3 자동 경로에서 유용).
- **참조횟수**: 0

### [2026-04-25] AI→SVG 미변환 알림 위치: PatternManage 상단 배너 (전역 헤더/토스트/Settings 거부)
- **분류**: decision
- **발견자**: planner-architect
- **내용**: 미변환 AI 감지 시 알림 UI 배치. **채택 (A)**: PatternManage 페이지 상단 노란 배너(role="alert"), `unconvertedAi.length > 0`일 때만 렌더. **거부 (B)**: 전역 헤더 배지 — UpdateModal과 경쟁, 레이아웃 전역 수정. **거부 (C)**: 토스트 알림 — 놓치면 재진입 경로 없음. **거부 (D)**: Settings 내 섹션 — 사용자가 설정에 들어가야 발견, 발견성 최악. **근거**: (1) 미변환 AI는 "패턴 등록" 도메인에 속하는 문제 → PatternManage가 자연스러움, (2) 배너는 unconvertedAi가 비면 자동으로 사라지므로 "닫기 버튼" 불필요, (3) SVG 표준화의 ⋮ 메뉴(카드 단위)와 공존 — 둘은 다른 수준(배너=전체, 메뉴=개별)이라 충돌 없음.
- **참조횟수**: 0

### [2026-04-25] AI→SVG Rust 커맨드: 신규 2개 추가 (run_python 직접 호출 거부)
- **분류**: decision
- **발견자**: planner-architect
- **내용**: Python `ai_converter` CLI 호출 방식. **채택 (A)**: `ai_convert_preview(files)`, `ai_convert_batch(files, overwrite)` 신규 Rust 커맨드 2개, 내부는 기존 `run_python` 재사용. **거부 (B)**: 프론트가 `invoke("run_python", { command: "ai_convert_batch", args: [...] })` 직접 호출 — 동적 문자열, 타입 체크 약화. **거부 (C)**: Python sidecar 전환 — SVG 표준화 decision 재확인: 배포 복잡도 2배, MSI 50MB+, 거부. **근거**: SVG 표준화 [2026-04-22] 결정과 동일 논리 재사용 — 타입 안전성, 명시적 네이밍, 바이브 코더 실수 방지. Python `run_python` 로직은 무변경(8개월 안정 동작).
- **참조횟수**: 0
