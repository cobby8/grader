...
### [2026-04-15] Drive 파서 유연화 + UI 폭 확장

📝 구현한 기능: 파일명 구분자 유연화(언더스코어/공백/하이픈 혼용 허용), 패턴명 trim 후처리, 카테고리 사이드바 폭 2배 확장. 빈 폴더 처리는 조사만.

| 파일 | 변경 내용 | 라인 |
|------|----------|------|
| `src/services/driveSync.ts` | SIZE_REGEX: `_` → `[\s_\-]+`, `.+` → `.+?`(non-greedy), `i` 플래그 추가 | 48 |
| `src/services/driveSync.ts` | parseFilename: 매칭된 presetName 끝의 구분자 문자 trim 후처리 추가 | 128~131 |
| `src/App.css` | `.pattern-layout__sidebar` `flex: 0 0 220px` → `440px` | 712 |

**빈 폴더 조사 결과** (수정 없음, 결론만 기록):
- 위치: `driveSync.ts` 395~410행(카테고리 등록), 465~472행(경고 push)
- 현재 동작: 빈 폴더도 **카테고리로 등록됨**(395~410행에서 children 유무와 무관하게 push). 단, 루트가 SVG 없고 하위도 없으면 등록 안 함. 별도로 465행에서 `warnings`에 `빈 폴더: {경로}` 만 push.
- 즉 "빈 중간 폴더"는 트리에 카테고리 노드로 표시되고 warning으로도 뜬다.
- **선택지**:
  - **(a) 유지**: 빈 폴더도 트리에 표시 (디자인팀이 앞으로 채울 예정인 폴더를 미리 보여주는 효과). 현재 동작.
  - **(b) 제외**: 빈 폴더는 카테고리에서 빼고 warning도 지움 → UI 깔끔, 단 "기획은 돼있지만 비어있는 폴더"가 안 보임.
  - **(c) 표시하되 dim 처리**: 카테고리는 등록하되 UI에서 비활성 스타일. 추가 UI 로직 필요.
- 권장: **(a) 유지** (현재 동작). Phase 1 범위에서 불필요한 추가 수정을 피하자는 원칙과도 부합.

**검증**:
- `npx tsc --noEmit`: PASS
- `npm run build` (vite): PASS (775ms, 72 modules, index.js 321.79kB)

💡 tester 참고:
- 테스트 방법: Drive 폴더에 `농구 유니폼 XL.svg`, `축구-유니폼-L.svg`, `야구_유니폼_XS.svg` 등 혼합 구분자 파일명을 넣고 스캔 → 모두 presetName 정상 추출되는지 확인.
- 정상 동작: 공백/언더스코어/하이픈이 섞여도 사이즈 토큰 앞 부분이 presetName으로 잡히고, 끝의 구분자는 제거됨.
- 주의할 입력: 사이즈 토큰이 파일명 중간에 있는 경우(예: `XL농구.svg`) → SIZE_REGEX는 `$` 앵커가 있어 매치 실패 → null 반환 (기존 동작 유지).
- UI: 카테고리 사이드바 폭이 220→440px로 넓어졌는지 육안 확인.
